...
\uncover<2->{
	...
	$\Rightarrow$ On average only a percentage of tries will lead to the desired result.
}
\end{frame}
%%%	
...
\begin{frame}{A Simple Example}
	...
	\uncover<2->{
		\vspace{0.5em}
		$\Rightarrow$ The network would need to generate an average of 16 block candidates before being able to extend the chain.
	}
...
\begin{frame}{Mining Bitcoin}
	...
	\uncover<2->{
		...
		\begin{itemize}
			\item Discard the block candidate
			\item Change the contents of the block header
			\item Calculate the new block header hash value
		\end{itemize}	
	}
	...
	\uncover<3->{
		...
		\begin{itemize}
			\item The block candidate is broadcast to the network
