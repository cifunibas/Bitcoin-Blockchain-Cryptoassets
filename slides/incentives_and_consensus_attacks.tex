...
\uncover<2->{
\textbf{Illustrative example} with R = 6.25 Bitcoin:
...
$ \Rightarrow \overline{r}$ per block: $6.25 \cdot \frac{1}{16}$ vs. $\frac{6.25}{3} \cdot \frac{3}{16}$.

\vspace{0.5 em}
$\Rightarrow \sigma_{r}$ per block:  $1.523$ vs. \color{focus} \textbf{$0.813$}
}	
...
\begin{itemize}
	...
	\item	In case of success, the Pool owns two block rewards.
\end{itemize}
...
\uncover<3->{
\vspace{0.5 em}
The faster the txs are propagated through the network, the higher their relative chances to be included in a valid block first:
}
...
\uncover<4->{
$\Rightarrow$ Relayed to a better connected node, the attack tx is probably getting confirmed first, leaving Raphael with coffee AND Bitcoin.}
