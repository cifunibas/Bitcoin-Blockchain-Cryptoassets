...
\footnotesize Steps:
	\begin{itemize}
		...
		\item Translate characters of country code into 9 +position in alphabet
		\item Perform mod97 operation and subtract from 98 to arrive at checksum
		\item I.e., 98 - 4835012345678009121700 mod 97 = 56
	\end{itemize}}
...
How much time would it take to find a pre-image that generates a specific hash using the cryptographic hash functions applied in Bitcoin with a try-and-error approach?
...
\uncover<2->{\textbf{Outcome:\\
 \color{focus}1\% \color{black}} chance to find a specific hash \textbf{within $7 * 10^{27}$ years}.}
...
\frametitle{References and Recommended Reading}
		...
		\begin{columns}[T]
			\begin{column}{0.1\textwidth}
					\includegraphics[width = 1.7cm, frame]{../assets/images/singh_cover}
			\end{column} %\hfill
	...
	\uncover<1->{
		\begin{columns}[T]
			\begin{column}{0.1\textwidth}
					\includegraphics[width = 1.7cm, frame]{../assets/images/katz_lindell_cover}
			\end{column} %\hfill
			\begin{column}{0.8\textwidth}
