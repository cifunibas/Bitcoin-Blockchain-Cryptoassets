...
%	\begin{figure}
%		\input{../assets/figures/application_for_asymmetric_cryptography}
%	\end{figure}
\end{frame}
%%%
...
\begin{frame}{RSA 1/4: Find Public Key}
	Alice chooses two primes \(p\) and \(q\) (e.g. \(p = 17\) and \(q = 11\)) and calculate
	\begin{align*}
		N &= p \cdot q \\
...
\begin{frame}{RSA 2/4: Encrypt Message}
	Bob converts message \(M\) into an integer (e.g. \(M =\) letter \(X\) in ASCII \(= 88\)) and calculates the encrypted message \(C\) using Alice's public key and the formula 
	\begin{align*}
		C &= M^e\ (mod\ N) \\
		\intertext{In our example:}
		C &= 88^7\ (mod\ 187) \\
...
\begin{frame}{RSA 3/4: Derive Private Key}
	Alice calculates private key \(k_{p}\) using the formula
	\begin{align*}
...
\begin{frame}{RSA 4/4: Decrypt Message}
	\begin{align*}
		\intertext{Alice decrypts Bob's message with formula}
		M &= C^{k_{p}}\ (mod\ N)
		\intertext{and calculates}
		M &= 11^{23}\ (mod\ 187) \\
		M &= 88 = X
		\text{\ in ASCII}
	\end{align*}
