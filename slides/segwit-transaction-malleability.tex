...
\begin{frame}{Sort Fork Implementation}
	...
	\uncover<3->{
		\textbf{If $r_{new} < r_{old}$:}\\
		\begin{itemize}
			\item \textbf{Nodes following $S_{old}$:} consensus relevant nodes (CRNs) create new blocks where they spend the \texttt{ANYONECANSPEND} outputs themselves.
...
\begin{frame}{Confirmation Capacity Increase}
	...
	$\rightarrow$ moving the redeem script to the witness field reduces the size of the transaction data.\\
	\vspace{0.5em}
	\uncover<2->{
