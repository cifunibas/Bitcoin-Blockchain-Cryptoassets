...
\textbf{Block withholding:} Purposeful delay of propagation of own valid candidate block to gain head start on next block.
\vspace{1.5em}	
}
...
\uncover<2->{
\textbf{Rule change:} Part of the network decides to alter the consensus rule set $S=S_{old}$ and proceed with an adapted rule set $S_{new}$.
\vspace{0.5em}
...
\uncover<3->{
$\Rightarrow$ Protocol-based forks may cause permanent splits.
}
...
\center \footnotesize
$P(\,b \in S_{new} \oplus b \in S_{old})\, = \dfrac{r_{old}}{R} \left( 1-\frac{|S_{new} \cap S_{old}|}{|S_{old}|}\right) + \dfrac{r_{new}}{R} \left( 1-\frac{|S_{new} \cap S_{old}|}{|S_{new}|}\right)$ 
...
\textbf{Security Tokens:} Multiple promises for the delivery of one good.
\vspace{1.5em}
