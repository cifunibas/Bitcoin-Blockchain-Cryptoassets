\begin{tikzpicture}[scale = 1.5]
	
	% define coordinates
	\coordinate (0) at (0,0);
	\coordinate (1) at (1,0);
	\coordinate (2) at (2,0);
	
	\node at (0){\includegraphics[height = 0.15\textheight]{../assets/images/agents/handing_money_right}};
	\node at (1){\alt<5>{\includegraphics[height = 0.15\textheight]{../assets/images/agents/intermediary_devil}}{\includegraphics[height = 0.15\textheight]{../assets/images/agents/intermediary}}};	
	\node at (2){\includegraphics[height = 0.15\textheight]{../assets/images/agents/reaching_left}};
	
\end{tikzpicture}